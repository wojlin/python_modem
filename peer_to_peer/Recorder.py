...
class Recorder:
    def __init__(self, logger: logging.Logger):
        ...
        self.__selected_audio_device: AudioDevice
    ...
    def get_audio_devices(self) -> List[AudioDevice]:
        ...
        for i in range(0, devices_amount):
            ...
            if channels > 0:
                ...
                audio_devices.append(AudioDevice(index=i, name=name, channels=channels))

        return audio_devices

    def set_audio_device(self, audio_device: AudioDevice):
        real_devices = self.get_audio_devices()
        found_device = any(real_device.index == audio_device.index for real_device in real_devices)
        if not found_device:
            raise ConnectionError("audio device not found")

        self.__selected_audio_device = audio_device
    ...
    def __setup_recording(self):
        self.__stream = self.__audio.open(format=self.__FORMAT,
                                          ...
                                          rate=self.__RATE,
                                          input=True,
                                          input_device_index=self.__selected_audio_device.index,
